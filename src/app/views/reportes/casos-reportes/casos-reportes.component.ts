...
@Component({
  selector: 'app-casos-reportes',
  templateUrl: './casos-reportes.component.html',
  styles: [`
...
`],
  styleUrls: ['./casos-reportes.component.scss'],
})

export class CasosReportesComponent implements OnInit {
  casoDialog: boolean;
  casos: any[];
  selectedCasos: any[];
  submitted: boolean;
  ...
  exportExcel() {
    ...
    this.casosService.downloadExcel(
      this.busqueda,
      this.sortField,
      this.sortOrder,
      ...
      .subscribe(response => {
        ...
        saveAs(blob, 'casos_export_' + new Date().getTime() + '.xlsx');
        this.loading = false
      }, error => {
